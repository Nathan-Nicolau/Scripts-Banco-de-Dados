...
INSERT INTO PRODUTO_PEDIDO (IDPEDIDO,IDPRODUTO,QUANTIDADE) VALUES
 ...
 select cliente.nome,produto.descricao from cliente join pedido on cliente.idcliente = pedido.idcliente
	join produto_pedido on pedido.idpedido = produto_pedido.idpedido
    join produto on produto_pedido.idproduto = produto.idproduto
    where produto.descricao = 'Chocolate';
    
/*2) Apresentar os produtos com valor abaixo de 1.00.*/

select produto.valor,produto.descricao from produto where produto.valor<1.00;

/*3) Apresentar todas a compras efetuadas para clientes de São Paulo.*/

select nome from cliente join pedido on cliente.idcliente = pedido.idcliente
	 where cliente.cidade = 'São Paulo' ;
	
/*4) Apresentar os vendedores venderam ouro.*/

select nome, produto.descricao from vendedor join pedido on vendedor.idvendedor = pedido.idvendedor
	join produto_pedido on produto_pedido.idpedido = pedido.idpedido
    join produto on produto_pedido.idproduto = produto.idproduto
    where produto.descricao = 'Ouro';
    
/*5) Apresentar os clientes que compraram do vendedor 'Josias'.*/

select cliente.nome from cliente join pedido on cliente.idcliente = pedido.idcliente
	join vendedor on pedido.idvendedor = vendedor.idvendedor
    where vendedor.nome = 'Josias';
    
/*6) Apresentar os clientes que compraram mais de 20 Vinhos no mesmo pedido.*/

select nome,produto.descricao from cliente join pedido on pedido.idcliente = cliente.idcliente
	join produto_pedido on produto_pedido.idpedido = pedido.idpedido
    join produto on produto.idproduto = produto_pedido.idproduto
    where produto_pedido.idproduto = '78' and produto_pedido.quantidade >20;
    
/*7) Apresentar os clientes que compraram Chocolate e Queijo no mesmo pedido.*/

select nome from cliente join pedido on pedido.idcliente = cliente.idcliente
	join produto_pedido on pedido.idpedido = produto_pedido.idpedido
    join produto on produto.id_produto = produto_pedido.id_produto;

SELECT *FROM PRODUTO
WHERE PRODUTO.DESCRICAO = "Chocolate";

/*Chocolate identificado com id 31*/

SELECT *FROM PRODUTO
WHERE PRODUTO.DESCRICAO = "Queijo";

/*Chocolate identificado com id 25*/

SELECT C.NOME FROM CLIENTE C JOIN PEDIDO PED 
ON PED.IDCLIENTE  = C.IDCLIENTE JOIN PRODUTO_PEDIDO PP
ON PP.IDPEDIDO = PED.IDPEDIDO JOIN PRODUTO PRO 
ON PRO.IDPRODUTO = PP.IDPRODUTO
WHERE PRO.IDPRODUTO = 25
AND C.NOME IN (
		SELECT C.NOME FROM CLIENTE C JOIN PEDIDO PED 
		ON PED.IDCLIENTE  = C.IDCLIENTE JOIN PRODUTO_PEDIDO PP
		ON PP.IDPEDIDO = PED.IDPEDIDO JOIN PRODUTO PRO 
		ON PRO.IDPRODUTO = PP.IDPRODUTO
		WHERE PRO.IDPRODUTO = 31
);
